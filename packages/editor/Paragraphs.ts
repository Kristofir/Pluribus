...
export function assertParagraphIds(doc: Node) {
  ...
  for (const p of paragraphRows(doc)) {
    if (
      ...
      throw new Error("Missing or duplicate paragraph identity");
    seen.add(p.id);
  }
}
...
export function prepareParagraphIds(tr: Transaction, token: () => string) {
  ...
  const preferred = new Map(
    paragraphRows(tr.before)
      .filter((p) => p.id)
      .map((p) => [p.id, tr.mapping.map(p.from, 1)]),
  );
  ...
  for (const p of rows) {
    ...
    if (
      ...
      (duplicate &&
        original !== undefined &&
        original !== p.from &&
        rows.some((other) => other.id === p.id && other.from === original))
    ) {
      const node = tr.doc.nodeAt(p.from)!;
      tr.setNodeMarkup(p.from, undefined, {
        ...node.attrs,
        paragraphId: token(),
      });
