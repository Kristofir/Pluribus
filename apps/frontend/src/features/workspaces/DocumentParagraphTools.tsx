  ...
  return (
    <details className="mt-6 border-t border-border pt-4">
      ...
      <p className="text-xs text-muted-fg my-3">
        Select saved paragraphs for an external agent. To link a card, select
        exactly one canvas element, then choose its paragraph.
      </p>
      {pending && (
      ...
      <ul className="space-y-3">
        {rows.data?.paragraphs
          ...
          .map((paragraph) => {
            const captured = passages.find(
              (p) =>
                p.documentId === documentId &&
                p.paragraphId === paragraph.paragraphId,
            );
            return (
              <li
              ...
              >
                <label className="flex items-start gap-2">
                  <input
                    type="checkbox"
                    checked={!!captured}
                    disabled={paused || rows.failed || (!captured && pending)}
                    onChange={(event) =>
                      onSelect(
                        {
                          documentId,
                ...
                {captured && captured.version !== rows.data?.version && (
                  <p className="text-xs mt-2">
                    Selected at version {captured.version}. Document changed;
                    deselect and select again to use the new version.
                  </p>
                )}
                <Button
                  intent="plain"
