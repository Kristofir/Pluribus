...
export async function readParagraphs(
  ...
  return {
    ...descriptor,
    version,
    paragraphs: paragraphRows(doc).map((p) => ({
      paragraphId: p.id ?? "",
      text: p.text,
      from: p.from + 1,
      to: p.to - 1,
...
export async function linkParagraph(
  ctx: MutationCtx,
  args: {
    workspaceId: Id<"workspaces">;
    elementId: Id<"canvasDocuments"> | Id<"sources">;
    documentId: Id<"documents">;
    paragraphId: string;
    version: number;
  },
  ...
  if (
    ...
    element.removed ||
    ("canvas" in element
      ? !("x" in element) || element.canvas !== args.workspaceId
      : element.workspaceId !== args.workspaceId)
  )
    throw new Error("Element unavailable");
  const current = await readParagraphs(ctx, args.documentId);
  if (current.canvasId !== args.workspaceId)
    throw new Error("Document belongs to another workspace");
  if (current.version !== args.version)
    throw new Error("Document changed; select the paragraph again");
  if (!current.paragraphs.some((p) => p.paragraphId === args.paragraphId))
  ...
  else await ctx.db.insert("documentLinks", value);
  return null;
}
