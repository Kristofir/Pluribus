...
async function setup() {
  ...
  const user = await t.run((ctx) => ctx.db.insert("users", {})),
    other = await t.run((ctx) => ctx.db.insert("users", {}));
  const workspaceId = await t.mutation(
    internal.workspaces.Provisioning.provision,
    { slug: "agents", name: "Agents", assignments: [] },
  );
  await t.run(async (ctx) => {
    await ctx.db.insert("workspaceMembers", { workspaceId, userId: user });
    await ctx.db.insert("workspaceMembers", { workspaceId, userId: other });
  });
...
test("agent request replay is immutable, stale version conflicts, revoked grants cannot read", async () => {
    ...
    edits: [
      {
        ...
        paragraphId: initial.paragraphs[0].paragraphId,
        text: "Agent text",
      },
    ],
  };
  const accepted = await t.mutation(
    internal.agentAccess.Tools.applyEdit,
    request,
  );
  expect(accepted.status).toBe("applied");
  expect(
    await t.mutation(internal.agentAccess.Tools.applyEdit, request),
  ).toEqual(accepted);
  expect(
    await t.mutation(internal.agentAccess.Tools.applyEdit, {
      ...request,
      requestId: uuid(),
    }),
  ...
  await expect(
    t.mutation(internal.agentAccess.Tools.applyEdit, {
      ...request,
      edits: [{ ...request.edits[0], text: "Changed request" }],
    }),
...
test("delegated complete-group undo preserves later human text elsewhere, refuses another initiating user", async () => {
  const { t, a, b, id, grant, read } = await setup(),
    initial = await read();
  const accepted = await t.mutation(internal.agentAccess.Tools.applyEdit, {
    token: grant.token,
    requestId: uuid(),
    documentId: id,
    generation: 1,
    ...
    edits: [
      ...
      { kind: "insert", afterParagraphId: null, text: "Agent heading" },
    ],
  });
  if (accepted.status !== "applied") throw new Error("Expected apply");
  const human = await b.mutation(api.Documents.openAuthorship, {
    id: `${id}:1`,
  });
  const current = ds.nodeFromJSON(
    await t.run(async (ctx) =>
      (await materialize(ctx, id, accepted.version)).doc.toJSON(),
    ),
  );
  const paragraph = ds.node(
    "paragraph",
    { paragraphId: uuid() },
    ds.text("Human addition", [
      ds.marks.authorship.create({ author: human.author }),
    ]),
  );
  const tr = new Transform(current).insert(current.content.size, paragraph);
  await b.mutation(api.Documents.submitSteps, {
    id: `${id}:1`,
    ...
    steps: tr.steps.map((s) =>
      JSON.stringify(new AuthoredStep(s, uuid()).toJSON()),
    ),
  });
  await expect(
    b.mutation(api.AgentAccess.undo, {
      changeId: accepted.operationGroupId,
      requestId: uuid(),
    }),
  ).rejects.toThrow("another user");
  const undo = { changeId: accepted.operationGroupId, requestId: uuid() };
  const result = await a.mutation(api.AgentAccess.undo, undo);
  expect(await a.mutation(api.AgentAccess.undo, undo)).toEqual(result);
  expect((await read()).paragraphs.map((p) => p.text)).toEqual([
    "",
    "Human addition",
  ]);
});
test("undo refuses to erase human text inserted inside an agent paragraph and rolls back all evidence", async () => {
  ...
  const tr = new Transform(current).insert(
    2,
    ds.text("human", [ds.marks.authorship.create({ author: human.author })]),
  );
  await b.mutation(api.Documents.submitSteps, {
    id: `${id}:1`,
    version: accepted.version,
  ...
  await expect(
    a.mutation(api.AgentAccess.undo, {
      changeId: accepted.operationGroupId,
      requestId: uuid(),
    }),
  ...
  expect(
    (
      await a.query(api.AgentAccess.changes, {
        workspaceId: (await t.run((ctx) =>
          ctx.db.get(accepted.operationGroupId),
        ))!.workspaceId,
      })
    )[0].undone,
  ).toBe(false);
...
test("paragraph links stay unresolved after deletion and replacement text, then resolve on verified Undo", async () => {
  const { t, a, id, grant, read, workspaceId } = await setup();
  const initial = await read(),
    anchor = initial.paragraphs[0].paragraphId;
  await t.mutation(internal.agentAccess.Tools.applyEdit, {
    token: grant.token,
    requestId: uuid(),
    documentId: id,
    ...
    edits: [
      { kind: "replace", paragraphId: anchor, text: "Linked paragraph" },
      { kind: "insert", afterParagraphId: anchor, text: "Keep" },
    ],
  });
  const linkedCard = await a.mutation(api.Canvas.createDocument, {
    workspaceId,
    geometry: { x: 0, y: 0, width: 100, height: 100 },
  });
  const before = await read();
  await a.mutation(api.Documents.linkParagraph, {
    workspaceId,
    elementId: linkedCard,
    documentId: id,
    paragraphId: anchor,
    version: before.version,
  });
  const deleted = await t.mutation(internal.agentAccess.Tools.applyEdit, {
    token: grant.token,
    requestId: uuid(),
    documentId: id,
    generation: 1,
    baseVersion: before.version,
    edits: [{ kind: "delete", paragraphId: anchor }],
  });
  if (deleted.status !== "applied") throw new Error("Expected deletion");
  expect(
    await a.query(api.Documents.resolveParagraph, {
      documentId: id,
      paragraphId: anchor,
    }),
  ).toBeNull();
  await t.mutation(internal.agentAccess.Tools.applyEdit, {
    ...
    edits: [
      { kind: "insert", afterParagraphId: null, text: "Linked paragraph" },
    ],
  });
  expect(
    await a.query(api.Documents.resolveParagraph, {
  ...
  await a.mutation(api.AgentAccess.undo, {
    changeId: deleted.operationGroupId,
    requestId: uuid(),
  });
  expect(
    await a.query(api.Documents.resolveParagraph, {
  ...
  ).toMatchObject({ text: "Linked paragraph" });
  expect(
    (await a.query(api.Documents.links, { workspaceId }))[0].paragraphId,
  ).toBe(anchor);
});
test("delegated undo refuses a human line break inside agent text", async () => {
  ...
  const tr = new Transform(current).insert(2, ds.node("hardBreak"));
  await b.mutation(api.Documents.submitSteps, {
    id: `${id}:1`,
    version: accepted.version,
    clientId: "human",
  ...
  ).rejects.toThrow("structural edit");
  expect(await a.query(api.Documents.latestVersion, { id: `${id}:1` })).toBe(
    before,
  );
});
test("accepted agent retry retains its outcome after a card delete/restore generation advance", async () => {
  ...
  const child = await a.mutation(api.Canvas.createDocument, {
    workspaceId,
  ...
  const secret = uuid(),
    ...
    action = uuid();
  const deleted = await a.mutation(api.Canvas.applyHistoryAction, {
    session,
    secret,
    action,
    attempt: uuid(),
...
test("agent paragraph Undo rejects a later human heading transformation", async () => {
  ...
  const accepted = await t.mutation(internal.agentAccess.Tools.applyEdit, {
    ...
    edits: [
      {
        kind: "insert",
        afterParagraphId: initial.paragraphs[0].paragraphId,
        text: "AB",
      },
    ],
  });
  ...
  const paragraph = current.child(1);
  const tr = new Transform(current).setNodeMarkup(
    current.child(0).nodeSize,
    ds.nodes.heading,
    { ...paragraph.attrs, level: 2 },
  );
  await b.mutation(api.Documents.submitSteps, {
    id: `${id}:1`,
    version: accepted.version,
...
test("retired rectangle context cannot be read or used for a new edit", async () => {
  const { t, a, workspaceId, id, grant, read } = await setup();
  const initial = await read();
  const contextSnapshotId = await a.mutation(api.AgentAccess.prepare, {
    workspaceId,
    elementIds: [],
    passages: [
      {
  ...
  await t.run((ctx) =>
    ctx.db.patch(contextSnapshotId, {
      content: JSON.stringify({
        elements: [{ kind: "rectangle" }],
        passages: [],
      }),
    }),
  );
  await expect(
    t.query(internal.agentAccess.Tools.context, {
      token: grant.token,
      contextSnapshotId,
    }),
...
test("stored rectangle paragraph links are omitted without deleting their evidence", async () => {
  const { t, a, workspaceId, id, read } = await setup();
  const initial = await read();
  const link = await t.run(async (ctx) => {
    const elementId = await ctx.db.insert("rectangles", {
      workspaceId,
      x: 0,
      y: 0,
      width: 100,
