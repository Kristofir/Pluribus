...
import { assertWorkspaceMembership, assertAdministrator } from "@pluribus/core/workspaces/access";
import type { QueryCtx } from "../_generated/server";
import type { Id } from "../_generated/dataModel";

/** Resolve both membership and the resource from storage on every request. */
export async function requireWorkspace(ctx: QueryCtx, workspaceId: Id<"workspaces">) {
...
export async function requireWorkspaceMember(ctx: QueryCtx, workspaceId: Id<"workspaces">, userId: Id<"users"> | null) {
  const member = userId ? await ctx.db.query("workspaceMembers").withIndex("by_workspace_user", q => q.eq("workspaceId", workspaceId).eq("userId", userId)).unique() : null;
...
export async function requireAdmin(ctx: QueryCtx) {
  ...
  const admin = userId ? await ctx.db.query("administrators").withIndex("by_user", q => q.eq("userId", userId)).unique() : null;
  assertAdministrator(userId, admin);
}
/** Absence deliberately selects the old public demo; no private record falls back to it. */
export async function canvasScope(ctx: QueryCtx, workspaceId?: Id<"workspaces">) {
