...
async function setup() {
  ...
  register(t);
  const rectangle = await t.mutation(api.Canvas.create, {
    geometry,
    color: "gold",
  });
  const document = await t.mutation(api.Canvas.createDocument, { geometry });
  return { t, rectangle, document };
}
test("live mixed-group writes coalesce into one receipt; Undo/Redo restore authoritative geometry atomically", async () => {
  const { t, rectangle, document } = await setup();
  const proof = credential();
  const updates = [rectangle, document].map((id) => ({
    id,
    generation: 1,
  ...
  expect(await t.query(api.Canvas.list, {})).toMatchObject([{ ...geometry }]);
...
test("a collaborator changing any group member prevents every Undo write", async () => {
  ...
  await t.mutation(api.Canvas.applyGeometry, {
    ...
    sequence: 1,
    final: true,
    updates: [rectangle, document].map((id) => ({
      id,
      generation: 1,
  ...
  expect(await t.query(api.Canvas.list, {})).toMatchObject([{ x: 40 }]);
  expect(await t.query(api.Canvas.documentCards, {})).toMatchObject([
    { geometry: { x: 90 } },
  ]);
});
test("rejected batches write neither partial geometry nor receipts; no-op gestures are not history actions", async () => {
  ...
  await expect(
    t.mutation(api.Canvas.applyGeometry, {
      ...
      final: true,
      updates: [
        { id: rectangle, generation: 1, geometry: { ...geometry, x: 30 } },
        { id: document, generation: 1, geometry: { ...geometry, width: -1 } },
      ],
    }),
  ).rejects.toThrow();
  expect(await t.query(api.Canvas.list, {})).toMatchObject([{ x: 0 }]);
  expect(
    await t.run((ctx) =>
      ctx.db
...
test("an interrupted stream seals the last submitted sequence without replaying stale movement", async () => {
  ...
  const first = {
    ...
    final: false,
    updates: [
      { id: rectangle, generation: 1, geometry: { ...geometry, x: 20 } },
    ],
  };
...
test("successive gestures on the same Element can undo and redo in sequence", async () => {
  ...
  for (const [proof, x] of [
    ...
    await t.mutation(api.Canvas.applyGeometry, {
      ...
      sequence: 1,
      final: true,
      updates: [{ id: rectangle, generation: 1, geometry: { ...geometry, x } }],
    });
  ...
  expect(await t.query(api.Canvas.list, {})).toMatchObject([{ x: 20 }]);
});
