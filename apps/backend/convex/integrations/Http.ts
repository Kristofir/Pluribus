/** Carries an HTTP status across adapters without exposing provider response bodies. */
...
export async function providerJson(
  ...
  while (true) {
    ...
    if (size > maxBytes) {
      await reader.cancel();
      throw new Error("Provider result exceeds size limit");
    }
    chunks.push(value);
  }
...
export function text(value: unknown, max = 100000) {
