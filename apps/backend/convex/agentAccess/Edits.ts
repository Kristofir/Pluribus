import { assertActiveContext } from "./Context";
...
export async function readAgentDocument(
  ctx: QueryCtx,
  args: { token: string; documentId: Id<"documents"> },
) {
  ...
  if (document.workspaceId !== grant.workspaceId || !document.element)
    throw new Error("Document outside grant");
  ...
  return {
    ...
    paragraphs: paragraphRows(
      (await materialize(ctx, document._id, version)).doc,
    ).map((p) => ({ paragraphId: p.id, text: p.text })),
  };
}
...
export async function applyAgentEdit(
  ctx: MutationCtx,
  args: Infer<typeof applyRequest>,
) {
  const grant = await requireGrant(ctx, args.token, args.documentId);
  const document = await requireDocument(
    ctx,
  ...
  if (document.workspaceId !== grant.workspaceId || !document.paragraphs)
    throw new Error("Document outside grant");
  ...
  const prior = await ctx.db
    .query("agentRequests")
    .withIndex("by_grant_request", (q) =>
      q.eq("grantId", grant._id).eq("requestId", args.requestId),
    )
    .unique();
  if (prior) {
  ...
  await requireDocument(
    ctx,
    `${args.documentId}:${args.generation}`,
    true,
    grant.userId,
  );
  ...
  if (version === null) throw new Error("Document unavailable");
  if (version !== args.baseVersion) {
    const result = { status: "conflict" as const, currentVersion: version };
  ...
  const tr = new Transform((await materialize(ctx, document._id, version)).doc),
    mark = schema.marks.authorship.create({ author });
    ...
    if (edit.kind === "insert") {
      const previous =
        edit.afterParagraphId === null
          ? null
          : rows.find((p) => p.id === edit.afterParagraphId);
      if (edit.afterParagraphId !== null && !previous)
      ...
      tr.insert(
        ...
        schema.node(
          ...
          edit.text ? schema.text(edit.text, [mark]) : undefined,
        ),
      );
    } else {
      const target = rows.find((p) => p.id === edit.paragraphId);
      if (!target) throw new Error("Paragraph unavailable");
      if (edit.kind === "delete") {
        ...
        tr.replaceWith(
          target.from + 1,
          target.to - 1,
          edit.text ? schema.text(edit.text, [mark]) : [],
        );
    }
  }
  const originalParagraphs = new Set(paragraphRows(tr.before).map((p) => p.id));
  const createdParagraphIds = paragraphRows(tr.doc)
    .filter((p) => !originalParagraphs.has(p.id))
    .map((p) => p.id);
  const createdStructures = paragraphRows(tr.doc)
    .filter((p) => createdParagraphIds.includes(p.id))
    .map((p) => {
  ...
  const result = await submitDocumentSteps(
    ctx,
    {
      id: scope,
      version,
      clientId: `agent:${grant._id}`,
      steps: steps.map((s) => JSON.stringify(s.toJSON())),
      protocol: 1,
    },
    { userId: grant.userId, author, session },
  );
  if (result.status !== "synced")
    throw new Error("Unexpected document conflict");
  const operationGroupId = await ctx.db.insert("agentChanges", {
