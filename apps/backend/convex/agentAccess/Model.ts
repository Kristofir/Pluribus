...
export const edit = v.union(
  v.object({
    kind: v.literal("replace"),
    paragraphId: v.string(),
    text: v.string(),
  }),
  v.object({ kind: v.literal("delete"), paragraphId: v.string() }),
  v.object({
    kind: v.literal("insert"),
    afterParagraphId: v.union(v.string(), v.null()),
    text: v.string(),
  }),
);
export const applyArgs = {
  ...
  edits: v.array(edit),
  contextSnapshotId: v.optional(v.id("agentContexts")),
};
export const applyResult = v.union(
  v.object({
