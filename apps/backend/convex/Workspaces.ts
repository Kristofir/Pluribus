...
export const admin = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.table === "users") {
      ...
      return {
        ...
        page: result.page.map((r) => ({
          id: String(r._id),
          label: r.name ?? r.email ?? "Unnamed user",
          workspaceId: null,
          userId: String(r._id),
        })),
      };
    }
    if (args.table === "workspaces") {
      const result = await ctx.db.query("workspaces").paginate(opts);
      return {
        isDone: result.isDone,
        continueCursor: result.continueCursor,
        page: result.page.map((r) => ({
    ...
    const result = await ctx.db.query("workspaceMembers").paginate(opts);
    return {
      isDone: result.isDone,
      continueCursor: result.continueCursor,
      page: result.page.map((r) => ({
